...
local abs = TL.extern('ind', TL.arrow(TL.num, TL.num), cmath.fabs)
local sgn = TL.extern('sgn', TL.arrow(TL.num,TL.num),
  terra( x : double )
    if x < 0.0 then return -1.0 else return 1.0 end
  end)
local ind = TL.extern('ind', TL.arrow(TL.bool,TL.num),
  terra( test : bool )
...
local tensl orient2d( p0:TL.vec2, p1:TL.vec2, p2:TL.vec2 )
  var e01 = p1 - p0
  var e02 = p2 - p0
  return e01[0]*e02[1] - e01[1]*e02[0]
end

local tensl edge_edge_isct2d(
  e0p0 : TL.vec2,   e0p1 : TL.vec2,
  e1p0 : TL.vec2,   e1p1 : TL.vec2
)
  ...
  -- one obvious limitation of the current Tenslang is that
...
local tensl orient3d( p0:TL.vec3, p1:TL.vec3, p2:TL.vec3, p3:TL.vec3 )
  var e01 = p1 - p0
  var e02 = p2 - p0
  var e03 = p3 - p0
...
local tensl edge_tri_isct3d(
  ep0 : TL.vec3,   ep1 : TL.vec3,
  tp0 : TL.vec3,   tp1 : TL.vec3,   tp2 : TL.vec3
)
