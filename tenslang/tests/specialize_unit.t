...
local vec   = Lib.Constant(T.vec3, {1,2,3})
local tval  = true
...
local multi_inlineAST = A.Function:New {
  ...
  body={ A.DeclStmt:New {
    ...
    expr = A.TensorLiteral:New{ value = vec:get(), valuetype = T.vec3 }
  }, A.DeclStmt:New {
    name = misymY,
...
test.fail(function()
  local mod = {}
  local tensl foo() var x = mod.a end
