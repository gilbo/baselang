local F = {}
package.loaded["tenslang.src.functions"] = F
...
local TC = require 'tenslang.src.typechecker'
local CG = require 'tenslang.src.codegen'

local Stats = require 'tenslang.src.stats'
...
local function basetype_string(typ)
      if typ == T.num     then return 'double'
  elseif typ == T.bool    then return 'bool'
  else error('INTERNAL: unexpected primitive type: '..tostring(typ)) end
end

local function add_struct_def(typ, struct_defs)
  assert(typ:istensor())

  local sname     = tostring(typ:terratype())
  local btypname  = basetype_string(typ:basetype())
  local size      = tostring(typ.dims[1] * typ.strides[1])
  local strdef    = 'typedef struct '..sname..' { '..
                      btypname..' d['..size..']; } '..sname..';'

  struct_defs[typ] = strdef
end

local function valtyp_string(typ)
  if typ:istensor() then
    return tostring(typ:terratype())
  else
    return basetype_string(typ)
  end
end

local function generate_signature(fname, func, struct_defs)
  ...
  for arg_i,typ in ipairs(atyps) do
    if typ:istensor() then
      add_struct_def(typ, struct_defs)
    end
  ...
  if #rtyps > 0 then
    if rtyps[1]:istensor() then
      add_struct_def(rtyps[1], struct_defs)
    end
...
local function generate_header(headerfile, func_defs, struct_defs)
  local stdincludes =
    '#include "stdbool.h"\n'..
    '#include "stdint.h"\n'
  ...
  local sdefs = ''
  local fdefs = ''
  for _,s in pairs(struct_defs) do sdefs = sdefs..s..'\n' end
...
function F.compiletofile(objfile, headerfile, ftable)
  ...
  for key,func in pairs(ftable) do
    ...
    if not isfunction(func) then
      error('must supply Tenslang functions as table values: '..
            tostring(key), 2)
    end
    ...
    table.insert(c_func_defs, generate_signature(key, func, struct_defs))
  ...
  if headerfile then
    local headercontents =
      generate_header(headerfile, c_func_defs, struct_defs)
