local S = {}
package.loaded["tenslang.src.specializer"] = S
...
local function constant_to_ast(cval, typ, anchor)
  if typ:isprimitive() then
    if typ:isnumeric() then
      return A.Number:NewFrom(anchor, { value=cval })
  ...
  elseif typ:istensor() then
    return A.TensorLiteral:NewFrom(anchor, { value=cval, valuetype=typ })
  else
...
function A.TensorMap:specialize(ctxt)
  ctxt:enterblock()
  local namesyms = {}
  for i,str in ipairs(self.names) do
    namesyms[i] = introsym(str, ctxt)
  end
  local expr = self.expr:specialize(ctxt)
  ctxt:leaveblock()

  return A.TensorMap:NewFrom(self, { names=namesyms, expr=expr })
end
function A.TensorFold:specialize(ctxt)
  ctxt:enterblock()
  local namesyms = {}
  for i,str in ipairs(self.names) do
    namesyms[i] = introsym(str, ctxt)
  ...
  return A.TensorFold:NewFrom(self, { names=namesyms, op=self.op, expr=expr })
end
...
function A.Lookup:specialize(ctxt)
  ...
  if A.LuaObj:isproto(base) then
    ...
    if type(obj) == 'table' and #args == 1 and A.String:isproto(args[1]) then
