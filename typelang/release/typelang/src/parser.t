local P = {}
package.loaded["typelang.src.parser"] = P

local T     = require 'typelang.src.types'
local A     = require 'typelang.src.ast'
local pratt = require 'typelang.src.pratt'
...
local function sqaccess(P, lhs)
  ...
  return A.Lookup:New({ base=lhs, arg=expr }, P, openbracket)
end
...
local function dotaccess(P, lhs)
  ...
  return A.Lookup:New({ base=lhs, arg=strnode }, P, dot)
end
...
function Lang.simpleexpr(P)
  ...
  elseif P:matches(P.number) then
    ...
    -- convert from terra to this language's primitive type
    local valtyp   = T.fromterratype(numtoken.valuetype)
    if valtyp == nil then
      P:error('unrecognized numeric literal type: '..
              tostring(numtoken.valuetype))
    end
    return A.Number:New({
