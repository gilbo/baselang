local A = {}
package.loaded["plexlang.src.ast"] = A
...
local Shapes = require 'plexlang.src.shape'
...
A.Expression:NewKind('Number',  {'value', 'valuetype'},
                                { value='number', valuetype=Maybe('type') })
A.Expression:NewKind('TensorLiteral',  {'value', 'valuetype'},
                                { value='table', valuetype=Maybe('type') })
...
-- "MultiLookup" handles base[arg1,arg2,...] accesses
A.Expression:NewKind('Lookup',  {'base', 'args'},
...
-- tensor indexing nodes
A.Expression:NewKind('TensorMap', {'names','expr'},
                                { names={'symbol'}, expr='Expression' })
A.Expression:NewKind('TensorFold', {'names','op','expr'},
                                { names={'symbol'}, op='string',
                                  expr='Expression' })
...
--    By contrast, note that the base language has relatively complicated
-- numeric and tensor type systems, casts, (non-recursive) function calls,
-- multi-assignments and multiple return values from functions.
--    While a simpler version of the language could omit these features,
-- they're sufficiently non-trivial to build and broadly useful that I
-- decided it would be preferrable to remove them from a skeleton language
-- rather than add them in.
--    This may or may not have been a sound design decision.
