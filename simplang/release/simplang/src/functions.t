local F = {}
package.loaded["simplang.src.functions"] = F
...
local TC = require 'simplang.src.typechecker'
local CG = require 'simplang.src.codegen'

local Stats = require 'simplang.src.stats'
...
function F.compiletofile(objfile, headerfile, ftable)
  ...
  for key,func in pairs(ftable) do
    ...
    if not isfunction(func) then
      error('must supply Simplang functions as table values: '..
            tostring(key), 2)
    end
  ...
  if headerfile then
    local headercontents = generate_header(headerfile, c_func_defs)
